...
class ClockHelper : LowPowerDevice
{
	...
	public:
		...
		virtual bool SetTime(const uint8_t &sec, const uint8_t &min, const uint8_t &hour, const uint8_t &day, const uint8_t &dow, const uint8_t &month, const uint16_t &year, const int32_t &utcOffset) = 0;
		virtual uint32_t GetSeed() = 0;
		virtual uint64_t GetUnixSeconds() = 0;
};
